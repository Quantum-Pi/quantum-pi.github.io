<script lang="ts">
    ...
    const keyToCols: Record<ScreenshotImageDictKey, {gallery: string, container: string}> = {
        acs: {gallery: 'grid-cols-4', container: ''},
        genshin: {gallery: 'grid-cols-4', container: ''},
        hfw: {gallery: 'grid-cols-8', container: 'col-span-2'},
    }
</script>
<div class="mx-32 grid grid-cols-2">
    ...
    {#each Object.entries(screenshotImageDict).sort((a, b) => a.length - b.length) as [key, value]}
        <div class={`m-4 ${keyToCols[key as ScreenshotImageDictKey].container}`}>
            <div class="text-3xl mb-4">{keyToName[key as ScreenshotImageDictKey]}</div>
