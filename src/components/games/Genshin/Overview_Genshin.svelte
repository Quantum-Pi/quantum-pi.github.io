<script lang="ts">
	...
	const weaponMaxCache = new Map<WeaponCacheKey, {level: number, refinement: number, added: boolean}>();
	...
	const filterMiniWeapon = (w: GenshinWeapon) => {
		...
		if (valid) {
			if (weaponMaxCache.has(w.key)) {
				...
				if (cache && (cache.level < w.level || (cache.level === w.level && cache.refinement < w.refinement))) {
...
<div>
	...
	<div class=" w-full mt-[-90vh] pb-[10vh] flex justify-center items-center flex-col">
		...
		<div
			class="w-fit h-full grid gap-6 grid-cols-4 sm:grid-cols-6 md:grid-cols-7 lg:grid-cols-9 xl:grid-cols-10 mb-8"
		>
			{#each weapons.filter(filterMiniWeapon).sort(sortMiniWeapon).filter(filterDuplicateWeapons) as w}
				<MiniWeapon weapon={w} />
			{/each}
		</div>
